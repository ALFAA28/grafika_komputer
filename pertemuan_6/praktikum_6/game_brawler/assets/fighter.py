...
class Fighter():
    # 🚨 PERUBAHAN: Menerima base_cooldown dari data
    def __init__(self, x, y, flip, data, sprite_source, animation_steps):
        # Data
        self.size = data[0]
        self.image_scale = data[1]
        self.offset = data[2]
        self.base_damage = data[3] # Damage dasar (Kekuatan Pukulan)
        self.base_cooldown = data[4] # 🚨 DATA BARU: Cooldown dasar karakter ini
        
        # 📢 LOGIKA SKALA DINAMIS (Teknik Membesar) 📢
        self.base_rect_width = 80    
        self.base_rect_height = 180  
        self.current_scale_multiplier = 1.0 
        self.is_enlarged = False # Status Teknik Membesar (Toggle)
        
        # State Animasi
        self.flip = flip
        self.animation_list = self.load_images(sprite_source, animation_steps)
        self.action = 0 # 0:diam #1:lari #2:loncat #3:serang1 (Katana) #4:serang2 (Teknik Khusus) #5:terkena #6:mati
        self.frame_index = 0
        self.update_time = pygame.time.get_ticks() 
        
        # Atribut Wajib Pygame & Fisika
    ...
    def load_images(self, list_of_sheets, animation_steps):
        ...
        for y, sheet in enumerate(list_of_sheets):
            ...
            for x in range(animation):
                temp_img = sheet.subsurface(x * frame_width, 0, frame_width, frame_height)
                temp_img_list.append(pygame.transform.scale(temp_img, (self.size * self.image_scale, self.size * self.image_scale)))
                
            animation_list.append(temp_img_list)
        return animation_list

    # 🚨 Fungsi untuk memperbarui hitbox
    def update_rect(self):
        ...
        # Pertahankan posisi bawah (lantai) dan tengah horizontal
        self.rect = pygame.Rect(old_center_x - new_width // 2, old_bottom - new_height, new_width, new_height)
        
    # 🚨 Fungsi untuk mengubah skala secara internal
    ...
    # 🚨 Fungsi untuk toggle Teknik Membesar
    def toggle_enlarge(self):
        # Hanya bisa toggle jika hidup, tidak sedang hit, dan tidak sedang menyerang
    ...
    def move(self, screen_width, screen_height, surface, target, controls=None): 
        ...
        SPEED = 10
        if controls is None: # BOT (Ronin)
            SPEED = 2 
        
        if self.alive and self.hit == False:
            
            # 📢 LOGIKA BOT (Fighter 2 / Ronin) 📢
            if controls is None:
                distance = target.rect.centerx - self.rect.centerx
                # Logika pergerakan dan serangan BOT
                if abs(distance) > 100:
                    dx = SPEED if distance > 0 else -SPEED
                    self.running = True
                    self.flip = distance < 0
                elif abs(distance) < 110:
                    self.attack_type = 1 
                    if self.attack_cooldown == 0:
                        self.attack(surface, target)
                    
            # 📢 LOGIKA PEMAIN (Fighter 1 / Komandan Samurai) 📢
            else: 
        ...
        # Apply gravity dan boundary check
        self.vel_y += GRAVITY
        dy += self.vel_y
    ...
    def attack(self, surface, target):
        ...
        cooldown_base = self.base_cooldown # 🚨 Gunakan Cooldown Dasar dari __init__
        
        # Cek apakah ini Komandan Samurai (yang punya fitur skala)
        is_player_commander = hasattr(self, 'is_enlarged') and self.base_cooldown == 15 

        if self.attack_type == 1:
            self.set_action(3)
            
            if is_player_commander == False: # Jika ini P2 (Ronin/Sensei - BOT)
                damage_dealt = damage_base 
                cooldown_time = cooldown_base # Menggunakan SENSEI_COOLDOWN (misal 90)
            else:
                # Damage Komandan Samurai (P1) DITINGKATKAN saat teknik Membesar aktif
                scale = self.current_scale_multiplier if self.is_enlarged else 1.0
                damage_dealt = int(damage_base * scale) 
                cooldown_time = cooldown_base # Menggunakan RONIN_COOLDOWN (misal 15)
        ...
        # Hitbox serangan (Juga diperbesar jika Komandan Samurai sedang membesar)
    ...
    def draw(self, surface):
        ...
        # Hitung posisi gambar agar tetap di tengah dan di lantai
        x_pos = self.rect.x - (self.offset[0] * final_scale)
        y_pos = self.rect.y - (self.offset[1] * final_scale)
        
        # Skala ulang gambar saat ini untuk tampilan
