...
# --- POIN 5: PENGGUNAAN ASSET 3D EKSTERNAL ---
# Mengimpor model kompleks (.obj) sebagai pengganti objek primitif
pet_db = {
...
# --- POIN 3: VIEWING & PROYEKSI (KAMERA) ---
# Mengatur sudut pandang kamera dalam ruang 3D (Proyeksi Perspektif)
...
def change_pet(d):
    ...
    # TRANSFORMATION: Rotasi pada preview
...
def start_game():
    ...
    # --- POIN 4: WARNA & ILUSI KEDALAMAN (CAHAYA & FOG) ---
    # Menggunakan cahaya dan kabut untuk menciptakan dimensi ruang
...
class Pet(Entity):
    ...
    def update(self):
        ...
        # --- POIN 2: TRANSFORMASI (ROTASI & TRANSLASI) ---
        dist = distance(self.position, self.target.position)
        self.look_at_2d(self.target.position, 'y') # Rotasi
        
        stop_range = 4 if self.target == player else 1
        if dist > stop_range:
            self.position += self.forward * self.speed * time.dt # Translasi
...
class Ball(Entity):
    ...
    def update(self):
        ...
        if self.y < 0.25:
            ...
            self.velocity.x *= 0.95; self.velocity.z *= 0.95
...
def input(key):
    ...
    if key == 'escape' and state == GameState.PLAYING:
        ...
        pause_ui.enabled = not mouse.locked
    ...
    if state == GameState.PLAYING:
        ...
        if key == 'e' and txt_interact.enabled:
